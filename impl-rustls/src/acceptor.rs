...
impl tls_api::TlsAcceptor for TlsAcceptor {
    ...
    fn builder_from_der_key(cert: &[u8], key: &[u8]) -> anyhow::Result<TlsAcceptorBuilder> {
        let cert: rustls::pki_types::CertificateDer = Box::leak(Box::new(cert)).to_owned().into();
        let pkey: rustls::pki_types::PrivatePkcs8KeyDer = Box::leak(Box::new(key)).to_owned().into();
        let pkey = rustls::pki_types::PrivateKeyDer::Pkcs8(pkey);
